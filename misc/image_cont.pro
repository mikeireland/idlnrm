...
; xtit=xtit,ytit=ytit,tit=tit, color=!d.n_colors-1 $
else contour,a,x,y,/noerase,/xst,/yst,$  ;Do the contour
          pos = [px(0),py(0), px(0)+swx,py(0)+swy],/dev,$
        c_color =  colors ,/nodata,xtit=xtit,ytit=ytit,tit=tit
;          pos = [px(0)/xs,py(0)/ys, (px(0)+swx)/xs,(py(0)+swy)/ys],/dev,$
;/nodata,xtit=xtit,ytit=ytit,tit=tit, color=!d.n_colors-1
;print, px(0), py(0), swx, swy
return
end

pro image_contPS, a, WINDOW_SCALE = window_scale, ASPECT = aspect, $
	INTERP = interp, nocontours=nocontours,xval=x,yval=y,xtit=xtit,$
        ytit=ytit,$
        tit=tit
...
if (!d.name eq 'X') then begin
 set_plot, 'ps'
 device, xsize=15, ysize=12
 image_contx, a, WINDOW_SCALE = window_scale, ASPECT = aspect, $
	INTERP = interp, nocontours=nocontours,xval=x,yval=y,xtit=xtit,$
        ytit=ytit,$
        tit=tit
 device, /close
