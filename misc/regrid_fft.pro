...
function regrid_fft, inarr,  dimx,  dimy, xrev = xrev,  yrev = yrev, trunc=trunc
a = inarr
inx =  (size(a))[1]
iny =  (size(a))[2]
...
if (keyword_set(trunc)) then truncarr = make_pupil(inx, inx*trunc) else truncarr=1
outft[0:inx-1, 0:iny-1] =  shift(fft(a, -1), inx/2, iny/2)*truncarr
