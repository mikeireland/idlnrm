...
;and Allen.
;NB: Although this is all about Janskys, the actual integrals are done
;d\lambda, i.e. Jy is converted to Fd\lambda by division by \lambda^2
...
;within the filter.
filt_l =  [0.36,0.41,0.47,0.55, 0.7,0.9, 1.22,1.63,2.19,3.45,  5,  10.2,21.0,25,60]
filt_jy = [1840,4240,4320,3832,2842,2246,1570,1020,636,313,182.5,42.7,10.0,8.7,1.3];J was 1724
zmag_flux = interpol(filt_jy,alog(filt_l),alog(wavelengths), /spline)
if keyword_set(filter) then begin
 w = [-1]
 if (filter eq 'J') then w = where(wavelengths ge 1.1  and wavelengths le 1.4 )
 if (filter eq 'Jc') then w = where(wavelengths ge 1.20  and wavelengths le 1.22 )
 if (filter eq 'H') then w = where(wavelengths ge 1.483 and wavelengths le 1.783)
 if (filter eq 'CH4S') then w = where(wavelengths ge 1.52 and wavelengths le 1.62)
 if (filter eq 'K') then w = where(wavelengths ge 1.995 and wavelengths le 2.395)
 if (filter eq 'Ks') then w = where(wavelengths ge 1.99 and wavelengths le 2.30)
 ...
 if (filter eq 'HST374') then w =  where(wavelengths gt 0.37 and wavelengths lt 0.38)
