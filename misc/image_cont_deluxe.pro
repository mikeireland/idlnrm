...
pro image_cont_deluxe, a, WINDOW_SCALE = window_scale, ASPECT = aspect, $
	INTERP = interp, nocontours=nocontours,xval=x,yval=y,xtit=xtit,$
        ...
        tit=tit,subtit=subtit,possubtit=possubtit,logim=logim,colsty=colsty,  ylog = ylog
;+
; NAME:
;	IMAGE_CONT
;
...
if (keyword_set(ylog) eq 0) then ylog=0
...
contour,a,x,y,/noerase,/xst,/yst,$	;Do the contour
	...
	/follow,c_lab=0,  ylog = ylog
if (n_ct gt 0) then $
contour,a,x,y,/noerase,/xst,/yst,$      ;Do the contour
           pos = [px(0),py(0), px(0)+swx,py(0)+swy],/dev,$
        c_color =  colors ,xtit=xtit,ytit=ytit,tit=tit,levels=nlevs ,$
	c_line=2,/follow,c_lab=0, ylog = ylog
;print,'neg'
           ...
           pos = [px(0),py(0), px(0)+swx,py(0)+swy],/dev,$
        c_color =  colors ,/nodata,xtit=xtit,ytit=ytit,tit=tit,levels=levs, ylog = ylog
