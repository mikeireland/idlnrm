...
for i =  0, nr-1 do for j = 0, nang-1 do begin
  ...
  endif else if (usevis eq 1) then begin
     modelv2 = binary_vis2data(params[*, i, j],vis2data=vis2data)
     chi2_vis[i, j] = total((modelv2.vis2data-vis2data.vis2data)^2/vis2data.vis2err^2)
  endif
endfor
...
if not keyword_set(highc_only) then begin
 ...
 if (usevis eq 2) then begin
  ...
  modelv2 =  binary_vis2data([bestp, 0.1, 0.1],vis2data = vis2data)
  bestchi2 += total((vis2data.vis2data - modelv2.vis2data)^2/vis2data.vis2err^2)
  modelv2 =  binary_vis2data([highp, 0.1, 0.1],vis2data = vis2data)
  highchi2 += total((vis2data.vis2data - modelv2.vis2data)^2/vis2data.vis2err^2)
 endif 
...
ic_jdm, exp(-(chi2_arr-min(chi2_arr))/2./sqrt(n_elements(t3data)/ndf)/4.), /noc,  tit = 'Lhood at Contrast: '+strtrim(init_crat, 2),$
...
;;Testing... can delete this.
;if (usevis eq 2) then begin
; test = binary_vis2atm([bestp, 0.1, 0.1],vis2data = vis2data)
; plot, vis2data.vis2data, psym=5, /ylog
; oploterror, vis2data.vis2data, vis2data.vis2err, psym=5
; oplot,test.vis2data
; wait, 1
;endif
...
significance = sqrt(chi2_null - chi2_bin)
print, significance,          format = '("Significance:         ",F7.2,"  = sqrt(chi2_null - chi2_bin)")'
if (p ne '') then printf,  1,  significance, format = '("Significance:        ",F7.2,"  = sqrt(chi2_null - chi2_bin)")'
