...
common t3block,  t3data,  apriori, vis2data, usevis,  cp_cinv,  proj,  proj_err, force_min_sep
  ...
  pfull =  [p, 0.1, 0.1]
  if (usevis eq 1) then modelv2 = binary_vis2data(double(pfull),vis2data=vis2data) $
  else if (usevis eq 2) then modelv2 = binary_vis2atm(double(pfull),vis2data=vis2data)
  ...
  if (usevis gt 0) then chi2 += total([double(vis2data.vis2data-modelv2.vis2data)/vis2data.vis2err]^2)
  if (arg_present(hessian) or arg_present(covar) or arg_present(sig)) then begin
    ...
    for i = 0, 2 do begin
     ...
     if (usevis eq 1) then begin 
      modelv2_1 = binary_vis2data(double(q),vis2data=vis2data)
      jacob_v[*, i] = (modelv2_1.vis2data -modelv2.vis2data)/delpar[i]
     endif else if (usevis eq 2) then begin
      modelv2_1 = binary_vis2atm(double(q),vis2data=vis2data)
      jacob_v[*,i] = (modelv2_1.vis2data -modelv2.vis2data)/delpar[i]
     endif
    endfor
