...
function freud,head, olog = olog,  ix = ix, trust_object_name=trust_object_name
...
if(strpos(iname,'NIRC2') ne -1) then begin
  ...
  if keyword_set(trust_object_name) then headinfo.source_name=strtrim(sxpar(head,'OBJECT'),2) $
  else headinfo.source_name=strtrim(sxpar(head,'TARGNAME'),2)
  headinfo.utc=strtrim(sxpar(head,'UTC'),2)
  headinfo.date=strtrim(sxpar(head,'DATE-OBS'),2)
  headinfo.jd=sxpar(head,'MJD-OBS')
  headinfo.elevation=sxpar(head,'EL')
