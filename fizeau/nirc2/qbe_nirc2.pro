...
pro qbe_nirc2, ddir, adate, flatfile, skies, frames=frames, tsize=tsize, fstart=fstart, nfs=nfs,$
    cal4src=cal4src, discard_sigma=discard_sigma, extn=extn, ddir_sky=ddir_sky,comments=comments, $
    setsquare=setsquare, median_sub=median_sub, trust_object_name=trust_object_name
...
noskyfit=0	       ; 0 = Chip-periphery sky subtraction [FOR FAINT SOURCES!]
		       ; 1 = NO Sky Subtraction (Recommended for Golay/subframe)
                                ; 2 = Don't even compute sky
                                ; bgr! (=1 still puts it in stats)
...
for cube_ix=0, n_cubes-1 do begin
  ...
  raw2cube_dither_nirc2,scframes,datadir=ddir, 		$ ; Input params
	...
	noskyfit=noskyfit,setsquare=setsquare,plotall=plotall,  $
        dcube = dcube,  skies = skies, extn=extn, skydir=ddir_sky, $
        /medsub_ssky, n_sigma=6.5, median_sub=median_sub, trust_object_name=trust_object_name
