...
pro makedithersky_nirc2,dcube,ssky,starmask=starmask,darks=darks,  nocheck = nocheck,  $
 gain = gain, shifts = shifts, handsky=handsky, medsub=medsub, divide_medsub=divide_medsub
...
if(keyword_set(divide_medsub) eq 0) then divide_medsub = 1
...
locs =  fltarr(2, nsky)
dcube_div = dcube
cube_meds = median(median(dcube,dim=2),dim=1)
if (divide_medsub) then begin
  for i=0,nsky-1 do dcube_div[*,*,i] /= cube_meds[i]
endif
mincube=min(dcube_div, dim=3)
if (nsky lt 3) then medcube = mincube $
else if (nsky lt 6) then $
 medcube = median([ [[mincube]], [[dcube_div]] ], dim=3) $
else if (nsky lt 8) then $
 medcube = median([ [[mincube]], [[mincube]],[[dcube_div]] ], dim=3) $
else begin
 mincubes = fltarr(dimx,dimy,nsky/2+1)
 for i=0,nsky/2 do mincubes[*,*,i]=mincube
 medcube = median([ [[mincubes]],[[dcube_div]] ], dim=3)
endelse
if (divide_medsub) then medcube *= median(cube_meds)
...
nused =  intarr(nsky)
for i = 0, nsky-1 do begin
 w =  where(sqrt((locs[0, *] - locs[0, i])^2 + (locs[1, *] - locs[1, i])^2) ge starmask/2.5)
