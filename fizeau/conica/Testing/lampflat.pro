...
; USAGE:
; 
;    mflat,start_num,end_num
;
; assumes CONICA format of alternate ON/OFF lamp flats

pro lampflat,start_num,end_num,flat,bad,cutoff, $
    prefx=prefx, sufx=sufx,datadir=datadir

if (keyword_set(start_num) eq 0) then begin
 print,'USAGE:'
 print,'mflat,start_num,end_num,flat,bad,datadir=datadir'
 return
endif
...
nflats=(end_num-start_num)/2.


for i=start_num,end_num,2 do begin
   filename=datadir+prefx+string(i,format="(I4.4)")+sufx
   flaton=float(reform(readfits(filename,head)))
   sz=size(flaton)
   filename=datadir+prefx+string(i+1,format="(I4.4)")+sufx
   flatoff=float(reform(readfits(filename,head)))
